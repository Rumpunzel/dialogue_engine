class_name OptionsTree, "res://dialogue_engine/assets/icons/icon_options_tree.svg"
...
func add_option(option_id:String, option_type = null):
	...
	if not option_type == null:
		...
		if not option_type.get("single_use", true) or (success_counter == 0 and failure_counter == 0) or (option_type.get("big_deal", false) and success_counter == 0 and new_option.check_success() >= DialogueOption.PASSED):
